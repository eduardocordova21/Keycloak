...
export class KeycloakAuthorizationService {
  ...
  public async hasEntitlement(resource: string, resourceId: string, scope: string): Promise<boolean> {
    ...
    try {
      const result = await this.httpClient.post(this.config.token_endpoint, body.toString(), { headers: new HttpHeaders().set('Content-Type', 'application/x-www-form-urlencoded'), }).toPromise<any>();
